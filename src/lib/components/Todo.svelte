...
<li class="border-2 border-primary-500 py-2 px-4 w-full !mb-4 shadow-lg flex justify-between">
    <div class="flex items-center">
        <input class="checkbox rounded-full border-2 border-primary-500 mr-4" type="checkbox" checked={todo.finished} on:change={updateTodoFinished} />
        <p class="{todo.finished ? 'line-through' : ''}">{todo.title}</p>
    </div>
    <button class="btn btn-icon !bg-gradient-to-br !from-primary-50 !to-primary-400 !text-slate-950 w-8 min-w-8" on:click={openModal}>
        i
    </button>
</li>
